...
# Link ONLY the true read-only inputs (the raw CSV and the .py modules). Every
# other artifact -- .pkl / .npy / .json / .html -- is regenerated by the run, and
# linking those would make a write follow the symlink and silently overwrite the
# committed copy in data/ or figures/.
for _d in SOURCE_DIRS:
    if _d not in sys.path:
        sys.path.append(_d)
    for _f in os.listdir(_d):
        if not _f.endswith(('.csv', '.py')):
            continue
